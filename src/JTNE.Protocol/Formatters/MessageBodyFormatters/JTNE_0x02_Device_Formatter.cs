...
{
    public class JTNE_0x02_Device_Formatter : IJTNEFormatter<JTNE_0x02_Device>
    {
        public JTNE_0x02_Device Deserialize(ReadOnlySpan<byte> bytes, out int readSize)
        {
            int offset = 0, bodyReadSize = 0;
            JTNE_0x02_Device jTNE_0X02 = new JTNE_0x02_Device();
            jTNE_0X02.Values = new Dictionary<byte, JTNE_0x02_Body_Device>();
            jTNE_0X02.CusotmValues = new Dictionary<byte, byte[]>();
            while (offset < bytes.Length)
            {
                byte typeCode = JTNEBinaryExtensions.ReadByteLittle(bytes, ref offset);
                if (JTNE_0x02_Body_Device.TypeCodes.TryGetValue(typeCode, out Type jTNE_0x02_BodyImpl))
                {
                ...
                else if (JTNE_0x02_CustomBody_Device.CustomTypeCodes.TryGetValue(typeCode, out Type jTNE_0x02_CustomBodyImpl))
                {
        ...
        public int Serialize(ref byte[] bytes, int offset, JTNE_0x02_Device value)
        {
            if (value.Values != null && value.Values.Count > 0)
            {
                foreach (var item in value.Values)
                {
                    if (JTNE_0x02_Body_Device.TypeCodes.TryGetValue(item.Key, out Type jTNE_0x02_BodyImpl))
                    {
                        var bodyImplFormatter = JTNEFormatterExtensions.GetFormatter(jTNE_0x02_BodyImpl);
            ...
            if (value.CusotmSerializeObjectValues != null && value.CusotmSerializeObjectValues.Count > 0)
            {
                ...
                {
                    if (JTNE_0x02_CustomBody_Device.CustomTypeCodes.TryGetValue(item.Key, out Type jTNE_0x02_CustomBodyImpl))
                    {
